...
import { academicAchieverPASCO } from '../../../../data/pascoAcademicAchieverData';
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-6 border border-gray-200">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 text-lg">{academicAchieverPASCO.tagline}</p>
            </div>
              ...
              <div className="relative w-36 h-36">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 64 * (1 - academicAchieverPASCO.overallScore / 100)}`}
                    strokeLinecap="round"
                  />
                  <defs>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <div className="text-4xl font-bold text-gray-900">{academicAchieverPASCO.overallScore}</div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'Learning Style', value: 'Visual', icon: Brain, color: 'bg-blue-50 border-blue-200', iconColor: 'text-blue-600' },
              { label: 'Social Energy', value: 'Extroverted', icon: Users, color: 'bg-cyan-50 border-cyan-200', iconColor: 'text-cyan-600' },
              { label: 'Study Preference', value: 'Small Group (2-3)', icon: Users, color: 'bg-indigo-50 border-indigo-200', iconColor: 'text-indigo-600' },
              { label: 'Motivation', value: 'Achievement', icon: Trophy, color: 'bg-emerald-50 border-emerald-200', iconColor: 'text-emerald-600' }
            ].map((stat, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
          ...
          <div className="mt-6 inline-flex items-center gap-2 bg-blue-50 px-5 py-3 rounded-full border border-blue-200">
            ...
            <span className="font-semibold text-gray-900">{academicAchieverPASCO.personalityType}</span>
...
function OverviewTab() {
  ...
  const radarData = academicAchieverPASCO.categories.find(c => c.name === 'Aptitude')?.attributes.map(attr => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-6">
        {academicAchieverPASCO.categories.map((category, index) => (
          <motion.div
            key={index}
      ...
      <div className="bg-blue-50 rounded-3xl shadow-xl p-8 border border-gray-200">
        ...
        <p className="text-gray-700 leading-relaxed mb-6 whitespace-pre-line text-lg">
          {academicAchieverPASCO.profileSummary.description}
        </p>

        <div className="flex flex-wrap gap-3 mb-6">
          {academicAchieverPASCO.profileSummary.strengths.map((strength, i) => (
            <motion.span
              key={i}
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
          ...
          <ul className="space-y-3">
            {academicAchieverPASCO.profileSummary.recommendations.map((rec, i) => (
...
function HistoryTab() {
  const chartData = academicAchieverPASCO.history.map(entry => ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Test History Cards */}
      <div className="grid grid-cols-2 gap-6">
        {academicAchieverPASCO.history.slice(0, 2).map((entry, index) => {
          const prevEntry = academicAchieverPASCO.history[index + 1];
...
function PerformanceTab() {
  const { peerComparison, studyGroupStats, academicIndex } = academicAchieverPASCO.performance;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-200">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-3xl font-bold text-blue-600">{academicIndex.score}/100</span>
          </div>
          <div className="h-5 bg-gray-100 rounded-full overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${academicIndex.score}%` }}
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { label: 'Focus Quality', value: academicIndex.focusQuality },
            { label: 'Challenge Completion', value: academicIndex.challengeCompletion },
            { label: 'Performance Consistency', value: academicIndex.performanceConsistency },
            { label: 'Competitive Ranking', value: academicIndex.competitiveRanking }
          ].map((metric, i) => (
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-200">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart
            data={academicAchieverPASCO.categories.map(c => ({
              name: c.name,
              score: c.score,
              target: 85
            }))}
          >
            ...
            <Bar dataKey="score" radius={[8, 8, 0, 0]}>
              {academicAchieverPASCO.categories.map((entry, index) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-200">
        ...
        <div className="space-y-4">
          {academicAchieverPASCO.timeline.map((activity, index) => {
      ...
      <div className="bg-indigo-50 rounded-3xl shadow-xl p-8 border border-gray-200">
        ...
        <div className="grid grid-cols-2 gap-4">
          {academicAchieverPASCO.upcomingMilestones.map((milestone, index) => (
            <motion.div
              key={index}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
