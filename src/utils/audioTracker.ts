// Simplified audio tracker to prevent duplicate playback v6.6
class AudioTracker {
  ...
  private constructor() {
    // Clear any old problematic session data on initialization
    this.clearOldSessionData();
  }

  private clearOldSessionData(): void {
    // Clear old format data that might be causing issues
    const oldData = sessionStorage.getItem('audioTrackerSession');
    if (oldData) {
      console.log('🧹 Clearing old audio session data');
  ...
  hasPlayed(key: string): boolean {
    ...
    if (lastPlayed && Date.now() - lastPlayed < 1000) {
      console.log('⏭️ AudioTracker v6.6: Recently played:', key, 'for', userType);
      return true;
    }
    ...
    if (played) {
      console.log('ℹ️ AudioTracker v6.6: Already played:', key, 'for', userType);
    }
  ...
  markAsPlayed(key: string): void {
    ...
    console.log('✅ AudioTracker v6.6: Playing:', key, 'for', userType);
  ...
  stopAllAudio(): void {
    console.log('🛑 AudioTracker v6.6: Stopping all active audio');
  ...
  reset(): void {
    console.log('🔄 AudioTracker v6.6: Resetting all tracking');
  ...
  resetForUser(userType?: string): void {
    ...
    console.log('🔄 AudioTracker v6.6: Resetting for user:', targetUser);
  ...
  debugInfo(): void {
    const userType = this.getCurrentUserType();
    console.log('🔍 AudioTracker v6.6 Debug:', {
      userType,
